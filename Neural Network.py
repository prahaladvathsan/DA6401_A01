...
class NAG(Optimizer):
    ...
    def update(self, weights, gradients):
        ...
        # Apply NAG update
        gradients = self.apply_weight_decay(weights, gradients, getattr(self, 'weight_decay', 0))
        return weights + self.momentum * velocity_prev + (1 - self.momentum) * self.velocity
...
class RMSProp(Optimizer):
    ...
    def update(self, weights, gradients):
        ...
        self.squared_grad = self.beta * self.squared_grad + (1 - self.beta) * (gradients ** 2)
        return weights - self.learning_rate * gradients / (np.sqrt(self.squared_grad) + self.epsilon)
